...
4.  **精准引用**: 在引用信息时，**必须**使用 Markdown 的数字引用语法 `[数字]` 来精准标注信息的来源。例如：`根据最新研究[1]，...` 或 `...显示了显著增长[2, 3]。`
    * 请为报告中引用的每个独特来源分配一个唯一的数字。
    * 如果某个信息来自多个来源，请列出所有相关来源的数字，如 `[1, 5, 7]`。
5.  **引用来源列表**: 在报告的**末尾**，请添加一个名为“引用来源”的部分。在此部分中，按照数字顺序，列出所有在报告中引用的 URL。格式为：
    ```
    引用来源：
    [1] URL_1
    [2] URL_2
